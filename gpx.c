...
{
    int rval = SUCCESS;
    fprintf(gpx->log, "DEBUG: port_handler(%p, %p, %p, %zd)\n", gpx, sio, buffer, length);
    if(length) {
        ssize_t bytes;
        int retry_count = 0;
        do {
            // send the packet
            fprintf(gpx->log, "DEBUG: Writing to port %d  %p %zd\n", sio->port, buffer, length);
            ...
            if(sio->bytes_in) {
                ...
                for(;;) {
                    ...
                    if((bytes = read(sio->port, gpx->buffer.in, 1)) == -1) {
                        ...
                        fprintf(gpx->log, "DEBUG: Read != 1 byte\n", errno);
                        return ESIOREAD;
                    }
                    fprintf(gpx->log, "DEBUG: Got %d want %d\n", (unsigned char)gpx->buffer.in[0], (unsigned char)0xD5);
                    // loop until we get a valid start byte
                    if(gpx->buffer.in[0] == (char)0xD5) {
                        fprintf(gpx->log, "DEBUG: Found 0xD5, breaking out first read\n");
                        break;
                    }
                }
                // read length
                ...
                if((bytes = read(sio->port, gpx->buffer.in + 1, 1)) == -1) {
                    ...
                    fprintf(gpx->log, "DEBUG: Wanted to read length of 1 got %zd\n", bytes);
                    return ESIOREAD;
                }
            }
            ...
            if((bytes = read(sio->port, gpx->buffer.in + 2, payload_length + 1)) == -1) {
                ...
                fprintf(gpx->log, "DEBUG: Failed to read expected length, got %zd\n", bytes);
                return ESIOREAD;
            }
            // check CRC
            unsigned crc = (unsigned char)gpx->buffer.in[2 + payload_length];
            fprintf(gpx->log, "DEBUG: Checking CRC, expecting %x\n", 0xFF & crc);
            ...
            fprintf(gpx->log, "DEBUG: CRC passed.\n");
            // check response code
            rval = gpx->buffer.in[2];
            fprintf(gpx->log, "DEBUG: Running %x through a switch of response codes\n", 0xff & gpx->buffer.in[2]);
            switch(gpx->buffer.in[2]) {
                ...
                case (char)0x8C:
                    ...
                    break;
                default:
                    fprintf(gpx->log, "DEBUG: Unknown reply code: %ud (%x)\n", (unsigned)gpx->buffer.in[2], 0xff & gpx->buffer.in[2]);
                    break;
            }
L_RETRY:
            // wait for 2 seconds
